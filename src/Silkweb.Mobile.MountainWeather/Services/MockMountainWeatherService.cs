    ...
    public class MockMountainWeatherService : IMountainWeatherService
    {
        ...
        public async Task<ForecastReport> GetAreaForecast(int id)
        {
            ...
            {                
                ...
                forecastReport.IssuedBy = (string)report["creating-authority"];
                forecastReport.IssueDateTime = issueDate;
                forecastReport.Title = (string)report["title"];
                forecastReport.Location = (string)report["location"];
                forecastReport.ValidFrom = DateTime.Parse((string)report["ValidFrom"]);
                forecastReport.ValidTo = DateTime.Parse((string)report["ValidTo"]);
                forecastReport.Validity = (string)report["Validity"];
                forecastReport.IssuedDate = (string)report["IssuedDate"];
                forecastReport.Hazards = report["Hazards"]["Hazard"].Select(hazard => new Hazard {
                    No = (short)hazard["no"],
                    Element = (string)hazard["Element"],
                    Risk = ((string)hazard["Risk"]).ToRisk(),
                    Comments = (string)hazard["Comments"]
                });
                forecastReport.Overview = (string)report["Overview"];
                forecastReport.ForecastDay0 = ParseForecast(report["Forecast_Day0"], issueDate.Date.AddDays(1));
                forecastReport.ForecastDay1 = ParseForecast(report["Forecast_Day1"], issueDate.Date.AddDays(2));
                forecastReport.OutlookDay2 = (string)report["Outlook_Day2"];
                forecastReport.OutlookDay3 = (string)report["Outlook_Day3"];
                forecastReport.OutlookDay4 = (string)report["Outlook_Day4"];
            }
            catch (Exception ex)
            {
